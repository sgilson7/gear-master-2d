...
- **There are two specializations** and you take one from a **trainer** — the
  Apothecary at a kiln wall in the Cairnworks, the Chef at a pot on three
  stones under the lake. One slot, outside the fork and outside the expert
  table. The Apothecary shipped with **no way to be taken at all**; see *The
  Apothecary shipped unreachable*.
- **The bank sorts** — by grid, by type, by set, and by any stat its rows
  carry, with the menu built from the rows rather than written out.
...
### High Wick comes down, and the post gets its name

**M22.** The third town is settled in three rungs off Kettleworks' own counter:
`nobody-has-named-it` sends you to confirm the post is empty, `send-for-the-clerk`
brings the High Wick clerk down, `the-long-mirror-inventory` opens volume ten and
brings the arcane shelf down with it, and `cut-the-post` puts the name on the
post. **Two wings, thirteen shelf lines, two commissions, three errands and one
name.**

- **The town keeps its own id and its own empty shelf.** High Wick's shelf is a
  *wing* of it — `high-wick` is still `high-wick`, its stock is keyed by its own
  id, and `bought`'s `(id, index)` contract is untouched. What the third town
  sells is what is standing in it, and `shop::counters_at` is the one answer.
- **The shelf lost three lines coming down, and that is what changed about it.**
  Its note called it *the only one that sells a book, an ink and a spell in the
  same room, which is the whole of what makes a caster weapon buildable at all*
  — written when High Wick was a town somebody would reach early. It is at the
  far end of two dungeons now, and the thing that makes a caster weapon
  buildable **from the first afternoon** is the barrel, which has carried a
  book, two spells and an orb since M20 for exactly that reason. So the
  `Chapbook`, the `Mirrorcast` and both `Rime Nova`s came off:
  `on_a_shelf_you_can_reach` read from the shelf's side. Nothing was ever
  bought there — it has been on no map since the fork — so the renumbering
  costs nobody anything, and **it can only be free once.**
- **What the clerk is sent for is a fight down there and a word about it**, and
  the plan asked for a `Bring` of *something the Undercountry drops*: nothing
  down there drops anything. A `Slay`'s token is created by the errand and
  cannot be had before it is asked for or after it is done, which is what *go
  down and come back with proof* wants. The token is `A Word About the Cellar`,
  one of six rumours the cut campaign left behind — on no shelf, granted by
  nothing, unreachable since the fork — so **no new component and no
  fingerprint move.**
- **`cut-the-post` is the first errand that has ever pointed at The
  Unwritten**, the longest fight in the game, which had no errand and no drops
  and nothing anywhere pointing at it. It pays a velvet-rope piece that was
  reachable from nowhere, which is the same move again.
- **A post can be given a name, and it is applied where the drains are.**
  `PlaceDef::named` is `{ when, name }` and `data::map_read_through` reads it —
  the one door every *game* question goes through. So `data::map` is the file
  and still says *a town with no name on the post yet*, `data::map_now` says
  **Low Wick**, and the eleven places in the shim that print `p.name` are right
  without being touched. That is `place_at`/`place_now`'s split, one field
  along; a `World::name_of` with one caller would have meant finding all eleven
  with no help from the compiler, because both are strings.
- **`UNWRITTEN` and `STAGED` are emptied and asserted empty, never deleted.**
  The assertion is what turns *nothing is staged* into a claim rather than an
  absence — a shelf that grew no ground under it, or a town that grew no shelf,
  would read as an oversight again, which is what both lists were written to
  stop. And four lints that asked *does this town trade* of the town's own
  shelf id had to learn to ask it of **the town and every wing of it**,
...
### The glass is a board, and the brew button is a moment

Reported from play: *"the brewing bench should be built using gear shaped items
like the surveying table, not using ench type items. then you should press a
brew button to actually turn your ingredients into a potion, that can be
activated before any fight."* Three changes, and the first is the one the other
two hang off.

- **The bench is the `Board` class**, the same one the packing screen and the
  instrument frame are drawn by, handed one grid through its api. It was a list
  of buttons — the ench rack's shape — which made an ingredient a line you tick
  rather than a shape you put somewhere, and the retort's whole point is that it
  is not a rectangle. Every rule is still core's: `brew::legal_anchors` answers
  where a thing may go, for the reason the green fit preview does.
- **`holes` is the one thing a gear grid has never needed.** The five worn
  frames are six wide and rectangular; the retort is seven cells in a four by
  three box. A hole is **not drawn at all** rather than shaded, because a shaded
  hole reads as a cell you failed to fill.
- **And the columns come off the payload.** `Board` had `6` written out in four
  places — the same *second copy of what grids exist* that `slotOrder` was
  before M11.6 fixed it one field along.
- **`Motif::Rose` was drawing nothing at all.** It has been the instrument
  frame's mark since M11.6 and `paintMotif` has never had an arm for it, so the
  one grid whose whole point is that it is not gear has been carrying **no shape
  channel** — a third of the colourblind triple missing, on the one screen never
  drawn beside another. Found by adding the flask beside it. Both are drawn now.
- **The button is the moment, and the potion is a thing.** The glass *was* the
  potion, which meant brewing never happened: you put two things in and they
  were silently on you at the next bell. Now `Game::brew` empties the glass and
  puts a potion in the pack, and `Game::drink` is a second decision that lands
  at the next bell — read out of `Character::drunk` and cleared beside the line
  in `fight::settle` that charges the fatigue. One at a time, and **a refusal
  spends nothing**.
- **A brew's id is derived from its pair**, sorted and joined, so a potion in
  the pack cannot be given an id that disagrees with what is in it.
- **What an ink added is on the potion, not in the file.** `BrewDef::ink_pct` is
  `#[serde(skip)]`: how strong a brew is, is a fact about the one somebody made
  rather than about the pair, and a file that named it would be a second answer.
...
## The Apothecary shipped unreachable, and a trainer is how you become one

**The largest finding since M20, and it is a shipped bug rather than a
measurement.** `Character::specialization` was written by exactly one line in
the whole repository — `save.rs`, the loader — so the Apothecary was a tree, a
power, a theme name and five honoured arms behind **no door at all**. Asked
directly *how do I get the first specialization?*, the answer was: you cannot.

**Two independent holes, and the second would have survived fixing the first.**
Nothing set the field; and `all_trees_json` filters on `Character::classes()`,
which is class, second class and expert — so even a character who somehow had
one could not spend a point in its tree.

- **`every_offered_class_reaches_something` could not see it**, because a
  specialization is deliberately outside `class::OFFERED`. *A lint that reads a
  list rather than the behaviour is the failure it exists to catch*, and the
  list it reads had a hole exactly the shape of the new feature.
  `every_specialization_has_a_trainer` asks the **maps** instead.
- **A comment beside a constant that moved.** The roster entry said *how you
  become one is a paper on Spike's van* — an intention, written while M20 was
  being built, that no line of code ever made true.
- **`Character::spendable_trees` is `classes()` plus the specialization**, and
  deliberately a second function rather than a wider `classes()`: everything
  that walks that list walks it to ask *which pair are you*, and a
  specialization pairs with nothing.

**How you become one is a trainer**, asked for in as many words: *each
specialization has one trainer you can find somewhere on the map, in hidden /
dangerous areas.* So it is a `Bench` with `teaches` and `stocks` — the whole of
*where* is content, and the next specialization moves no code.

- **A bench is two kinds of person now**, and the papers had to stop following
  the second one around: a trainer at the bottom of the Cairnworks was offering
  the Kaklon Patent. `licence` and `papers` are gated on the place **selling
  enchs**, which is the right question rather than an id — the licence is the
  licence to bolt on the things standing beside it.
- **`counter_here` was `sells` alone** and a trainer sells no enchs, so
  `train_here` found nobody standing in front of the player who was standing in
  front of them. It asks all three now.
- **A trainer is never on a table.** The Chef's was authored onto the
  Undercountry, which is a *shot* map: the arrow keys aim a cue, and `catches`
  is gates and bosses, so a counter there is a hole in one.
  `every_place_on_a_shot_map_is_reachable_by_shots` passes it and is right to —
  the tile *is* landable, which is the question that lint asks.
  `a_trainer_is_on_ground_you_walk` is the sharper one, narrow on purpose: a
  specialization is taken once, permanently, from one person in the world.
- **The bank's sort check was vacuous twice, and only breaking it found out.**
  It asserted *something moved*, which is an assertion about the bag rather
  than the sort; then *the shelf is grouped*, which two rows satisfy for
  nothing. Both passed with `sortedForBank` ripped out entirely. What it needed
  was a shelf worth sorting: `testing/saves/at-the-lip.json` stripped of its
...
- **And that plant opened the fork**, which is the one screen that does not
  ...
  *unlicensed character* trap and met again from the other side.
- **`clear_screens` closed each screen once, and screens stack.** Taking a
  trainer's offer opens the tree *over* the vendor; the vendor is earlier in
  that list, so its button was covered, its click failed, and the Escape that
  follows closed the tree instead — leaving the vendor up, where the generic
  Escape sweep cannot help because **the vendor is one of the few screens with
  no Escape handler at all**. The next check then failed against a screen it
  had never heard of, which is this harness's oldest failure mode with a second
  screen in it. The click pass runs twice now.
- **`an Apothecary`, `a Chef`.** One of the two starts with a vowel and the
  sentence sits on the screen where a permanent choice is confirmed.
  `class::an` is the one answer and `nobody_is_a_apothecary` asks it over
  `SPECIALIZATIONS`. *A glossary is a proofreading surface* — so is a receipt.

### Two halves of one bench, and they are opposites

Asked for: *the apothecary makes few ingredients more potent and unlocks unique
effects, and the chef can consume multiple potions per fight and unlocks
special ingredients they can buy from the trainer in both cases.*

| | Apothecary | Chef |
|---|---|---|
| buys | **depth** — one brew, worth more | **breadth** — two or three at once |
| power | `potency_pct`, `extra` cells of glass | `draughts`, how many you hold |
| honoured in | `Character::boon`, `Game::retort` | `Game::drink`, `Character::boon` |

A Chef who also made brews stronger would be an Apothecary with a hat on, and
the whole point of *you may only have one* is that the choice costs you the
other.

- **`drunk` is a list**, and it needed no new combat code: everything a brew
  gives goes through `Held`, which is the one door *what you are already
  holding when the bell goes* comes through, and two potions are two lots of
  the same addition. `Gives::and` adds rather than maxes, because a `max` would
  quietly make the second one free.
- **And it reads a lone string**, which is what the field was. Without
  `one_or_many` a save taken between brewing and the next fight would be
  **refused** — much the worst outcome available for a field absent from almost
  every save.
- **A trainer's ingredient is an ink and never half a pair**, which is what
  makes it cost the brew table nothing. `every_pair_of_ingredients_brews_to
  _something` is over all pairs, so two pairing ingredients on top of eight
  want **seventeen** new brews written and the next two want nineteen more. An
  ink multiplies instead, which is what the third slot has done since M20.
- **`BrewsData::parse` lets an ink have no `from`**, because a trainer's stock
  is not a drop — and that is a hole rather than a rule unless something asks
  the maps whether anybody stocks it.
  `every_special_ingredient_is_on_a_counter` is that half, and it refuses the
  other direction too: a thing you can farm *and* buy is a thing the farming is
  pointless for.
...
## The errand log is the skill tree

Reported from play twice, and the second report is the one that mattered: first
*"the current errand tree is just hard to follow as a player"*, which produced
the tree's **layout**; then *"the errands screen is really messed up, it should
look more like the skill tree"*, which is what happens when you give a tree's
layout the shelf's **cards**.

**It does not resemble the skill tree, it is the skill tree.** Same
`.skilltree` class, same centred `.tier` rows, same 208-pixel nodes, same wires
underneath, same `#node-detail` hover card — a card that already lives outside
...
- **Three lines on a node, the same three a skill node has**: the name is the
  ...
  errands in depth-ordered rows were fifty paragraphs with lines drawn between
  them, which is a worse screen than the flat list it replaced.
- **The brief, where it points and what it pays are in the card.** Which is also
  where hovering already rang the map, so one gesture now answers both
  questions at once.
- **The rung labels are gone.** Depth is conveyed by position, which is the
  tree's own answer and needs no furniture beside it.
- **And a check that read `.meta` off a node had to learn where that sentence
  went.** `check_the_log_points_somewhere` asks the hover card now. A check
  pinned to markup rather than to behaviour is a check that breaks when the
  screen is improved — which is the cheap half of this file's *a lint that reads
  a list rather than the behaviour* rule.

## The pack is on the screen where the decision is made

Reported from play: *"you should be able to drink the brew before battle in the
pre-battle screen, not in the brewing screen. they should be in a separate view
than the enchs."* Right twice over.

- **Brewing is a thing you do in a town and drinking is a thing you do in front
  of something.** The bench had both, which meant the decision a potion exists
  to create was being made in the one place there is nothing to make it about.
  `#potions` is on the pre-battle stage; the bench keeps a receipt of what is in
  the pack and offers no button at all.
- **Its own block, not a row in the rack.** A rack is where enchs live, and two
  unrelated lists under one heading is the `.card` collision in a new coat.
- **Hidden when there is nothing in it**, which is the rack's own rule: a list
  you cannot use is worse than no list.

## An errand log that is a tree
...
| 21.1 | **A crop turns, and `PLAN-M21.md` §M21.1 gives `plant(town, seed, at)` with no turn.** Measured before the rule was written: of the twenty-four seed-and-bed pairs, **three fit nowhere at all** — `toad-ichor` and `tallow-drip` in Kettleworks' gapped row, `reef-salt` in the pit's round bed. A seed that is dead in a town for a reason the player cannot act on is worse than a bed that is too easy, and `brew::Seat` has carried a `turn` since M20 because *a heuristic that refuses an arrangement somebody can see with their eyes* is what `brew::fits` exists not to be. | `crates/core/src/plot.rs`, `Crop::turn` |
...
| Ench effect kinds | **10**: power, haste, spin, fragile, heavy, **returns, ambush, coats, leeches_mana, ramps**. The five new ones are M21.6's and **four of them are mechanics the engine already had** — `Stats::reflect`, `RunningItem::overtakes`, `Trigger::OnActivate(Action::Curse)` — exposed as enchs for the first time, which is *before adding a system, grep for it* paying for itself. `the-one-page` needed no variant at all: it is `Fragile { pct: 400 }`. Only the mana leech and the ramp are new code |
...
| Enchs | **16** — 13 as before, plus **three the Plot grows**: a companion pair of crops touching at harvest pays an ench seed, which is the fourth source after a bench, a tree and an errand, and the first that is not one of those three. Three more wait for the Stall in M21.8, because an ench with no source is refused by `every_ench_comes_from_somewhere` |
...
| Specializations | **2** — the Apothecary and the Chef, and **you take one from a trainer**: `PlaceDef::teaches` on a bench, one each, in the Cairnworks and under the lake. They are the two halves of the bench and they are opposites — depth against breadth, `potency_pct` against `draughts` — because *you may only have one* is only a choice if it costs you the other. Neither is in `class::OFFERED`, neither pairs, and `Character::spendable_trees` is what lets their trees be spent in |
...
| Special ingredients | **2**, one a trainer, and they are **inks**: never half a pair, which is what makes them cost the brew table nothing — `every_pair_of_ingredients_brews_to_something` is over *all* pairs, so two pairing ingredients on top of eight want seventeen new brews and the next two want nineteen more. Sold at 400 to their own kind only |
