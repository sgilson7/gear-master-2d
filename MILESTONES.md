# The combined block — M16, M17, M18

*Two plans and a notebook. `PLAN-M16.md` is the Eleven Reefs and two classes;
`PLAN-M17.md` is the country as a table; M18 is the two notebooks executed.*

| # | Milestone | Deliverables | Status |
|---|---|---|---|
| **M16.0** | The fixture and the primitives | the run as a save; `common::from_save`; `Character::item_partition`; `quick` terrain; `MonsterSpec.enchs`; `no_creature_is_past_the_curse_cap` red then green | — |
| **M16.1** | The gate and the Flat Below | `the-way-under-the-flat` on the Sands; `the-reefs-1`; nine stakes, three bands, six pockets, the sheet; floors 2–3 stubbed | — |
| **M16.2** | The Assay | `the-reefs-2`; three doors; the assay office; the repack measured off the run | — |
| **M16.3** | The Needle Room and the Tenth Surveyor | `the-reefs-3`; four sinkholes, four levers, the plate; the boss wearing the run; the DPS bracket | — |
| **M16.4** | Two more base classes | `ClassPower::Stoker`, `ClassPower::Whisperer`; `OFFERED` at seven; two nine-node trees; the fork at seven cards | — |
| **M16.5** | Eleven experts | `EXPERTS` at twenty-one; eleven `ExpertPower` variants; eleven six-node trees; three new rules | — |
| **M16.6** | The M16 gate and walk | browser checks; the walk to the plate; `HANDOFF-M16.md`; `CLAUDE.md` | — |
| **M17.0** | The table, in core | `shot.rs` — `Shot`, `Flight`, `Contact`, `shoot`; `traversal` on `TilesData`; integer physics | — |
| **M17.1** | Obstacles | five `PlaceKind`s; parse lints; the Treyway as a table | — |
| **M17.2** | Landing | `Game::shoot`; `Step`'s resolution at rest; the landing roll; the tape | — |
| **M17.3** | The cue | the drag, the keyboard, the arrow, the flight, the strip, reduced motion | — |
| **M17.4** | The walker aims | `shot::aim_at`; the reachability lint; `playthrough.py` shooting | — |
| **M17.5** | The Undercountry, and the handoff | the second table; `HANDOFF-M17.md`; `CLAUDE.md` | — |
| **M18** | The notebooks executed | every `worklist` row in `SECOND-ORDER-M16.md` and `SECOND-ORDER-M17.md` turned into a milestone and resolved | — |
