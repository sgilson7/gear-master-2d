...
fn every_floor_in_the_game_can_be_solved_blind() {
    ...
    // **Six since M16**, and the sixth is the Flat Below. A list of five
    // written by hand is a list that can be four, which is the eighth time this
    // project has said that sentence.
    assert_eq!(walked, 6, "{walked} floors in the game have a card puzzle on them, not six");
