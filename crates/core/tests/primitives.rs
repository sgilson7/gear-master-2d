...
fn a_quest_item_is_carried_and_never_worn() {
    ...
    let mut ch = gm2d_core::character::Character::seeded(0x9E57_0001);
    let word = quest_items()[0].name;
    let id = ch.give(word).expect("the road can hand one over");
    ...
    for slot in SlotKind::ALL {
        for y in 0..4u8 {
            for x in 0..4u8 {
                assert!(
                    ch.can_equip(id, slot, x, y).is_err(),
    ...
    assert!(ch.inventory().contains(&id), "and it is still in the tray");
}
